 ...
 * @package TYPO3
 */
class tx_ccinfotablesmgm_encoding {
	 ...
	 */
	 function convertEncodingTable($table, $source_encoding, $dest_encoding='utf-8') {
		...
		if ($source_encoding AND $dest_encoding) {
		 	...
		 	$stringColumns = array();
		 	foreach ($GLOBALS['TCA'][$table]['columns'] as $field => $colDef) {
			...
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid,'.implode(',', $stringColumns), $table, '');
...
if (defined('TYPO3_MODE') && $GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/cc_infotablesmgm/class.tx_ccinfotablesmgm_encoding.php']) {
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/cc_infotablesmgm/class.tx_ccinfotablesmgm_encoding.php']);
}
?>